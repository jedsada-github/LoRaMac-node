...
static void LmhpFragmentationOnMcpsIndication( McpsIndication_t* mcpsIndication )
{
    ...
    uint8_t  blockAckDelay   = 0U; /* Co-efficient used to calculate delay. */
    uint8_t  fragIndex       = 0U;
    uint8_t  participants    = 0U;
    uint8_t  id              = 0U;
    uint8_t  status          = 0U;
    uint16_t fragCounter     = 0U;
    ...
    while( cmdIndex < mcpsIndication->BufferSize )
    {
        ...
        {
            fragIndex    = mcpsIndication->Buffer[cmdIndex++];
            participants = fragIndex & 0x01;
                ...
                ( ( participants == 0 ) && ( FragSessionData[fragIndex].FragDecoderStatus.FragNbLost > 0 ) ) )
            {
                ...
                LmhpFragmentationState.DataBuffer[dataBufferIndex++] =
                    ( fragIndex << 6 ) | ( ( FragSessionData[fragIndex].FragDecoderStatus.FragNbRx >> 8 ) & 0x3F );
                LmhpFragmentationState.DataBuffer[dataBufferIndex++] =
...
#if( FRAG_DECODER_FILE_HANDLING_NEW_API == 1 )
                FragDecoderInit( fragSessionData.FragGroupData.FragNb, fragSessionData.FragGroupData.FragSize,
                                 &LmhpFragmentationParams->DecoderCallbacks );
#else
                FragDecoderInit( fragSessionData.FragGroupData.FragNb, fragSessionData.FragGroupData.FragSize,
                                 LmhpFragmentationParams->Buffer, LmhpFragmentationParams->BufferSize );
#endif
            }
        ...
        case FRAGMENTATION_DATA_FRAGMENT:
        {
            ...
            if( mcpsIndication->Multicast == 1 )
            {
                ...
                // if( ( groupId == 0xFF ) ||
                //    ( ( FragSessionData[fragIndex].FragGroupData.FragSession.Fields.McGroupBitMask & ( 1 << groupId )
                //    ) == 0 ) )
                //{
                //    // Ignore message
                //    break;
    ...
    if( dataBufferIndex != 0 )
    {
        ...
        {
            /*
             * Delay value is calculated using BlockAckDelay which is communicated by server during the
             * FragSessionSetupReq Pseudo Random Delay = rand(0:1) * 2^(blockAckDelay + 4) Seconds. Delay = Pseudo
             * Random Delay * 1000 milli seconds. Eg: blockAckDelay = 7
